    ...
    for i in range(1, height + 1):
        for j in range(1, width + 1):
            if maze[i - 1][j - 1] == 0:
                ...
                pygame.draw.rect(
                    gameDisplay, black, [size * j, size * i, size, size], 1
                )
...
def generateRandomPopulation():
    ...
    for j in range(populationSize):
        ...
        for i in range(dimension):
            while True:
                print(actualLine,actualColumn)
                cell = random.choice(domain)
                if (
                    cell == "→"
                    and maze[actualLine][actualColumn + 1] != 0
                    and (
                        lastDirection != "←"
                        or (
                            maze[actualLine + 1][actualColumn] == 0
                            and maze[actualLine - 1][actualColumn] == 0
                            and maze[actualLine][actualColumn - 1] == 0
                        )
                    )
                ):
