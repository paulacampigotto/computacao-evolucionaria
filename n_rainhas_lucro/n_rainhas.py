...
class Individuo:
    ...
    def __init__(self, cromossomo):
        ...
        self.colisoes = self.calculaColisoes()
        self.lucro = self.calculaLucro()
        self.fitness = .37*self.lucroFitness() + .63*self.colisoes


    def getId(self):
        return self.id

    def lucroFitness(self):
        digitos = math.floor(math.log10(self.lucro))+1
        return self.lucro/(pow(10,digitos))
    ...
    def calculaLucro(self):
        ...
        for i in range(DIM):
            coluna = i
            linha = self.cromossomo[i]
            lucro+=matriz_lucro[linha][coluna]
        return lucro
...
def gera_matriz():
    ...
    for i in range(DIM):
        ...
        for j in range(DIM):
            if i%2 == 0:
                matriz[i].append(math.sqrt(cont))
            else:
                matriz[i].append(math.log10(cont))                
            cont+=1
    return matriz
